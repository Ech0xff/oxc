...
#[test]
fn comparison_type_argument_ambiguity_is_stack_safe() {
    let suffix = " | c > /x/".repeat(10_000);
    let source = format!("(a < b){suffix};\n");
    test_options_with_source_type(&source, &source, SourceType::ts(), default_options());
}
